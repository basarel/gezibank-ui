...
export default async function HotelLandingPage() {
  ...
  const popular_abroad_cities = widgets.filter(
    (x) => x.point === 'popular_places_abroad'
  )
    ...
    <div>
      ...
      <Container className='grid gap-3 py-5 md:gap-7 md:py-10'>
        <div>
          ...
          <div className='xs:w-[600px] flex w-[325px] gap-4 overflow-x-auto font-semibold sm:w-[680px] md:grid md:w-full md:grid-cols-3 md:gap-4'>
            {teaser &&
              teaser.length > 0 &&
              teaser
            ...
            <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
              {popular_domestic_cities.map((item) => (
                <ProductBox
                  ...
                  title={item.title}
                  url={item.params.link.value as Route}
                />
              ))}
            </div>
          </div>
