...
const CyprusSearchResult: React.FC<IProps> = ({
  ...
  return (
    <div className='my-2 rounded-lg border border-gray-300 shadow hover:shadow-xl'>
      <div className='grid md:grid-cols-12'>
        <div className='md:col-span-9'>
          <div className='grid gap-5 p-3 md:grid-cols-8'>
            ...
            <div className='flex flex-col gap-2 pt-2 md:col-span-5'>
              ...
              <div>
                <div>
                  <List size='sm' className='px-0'>
                    {roomDetail?.pensionType && (
                      <List.Item
                        className='px-1'
                        icon={<FaCheck color='black' size={'12'} />}
                      >
                        {roomDetail.pensionType}
                      </List.Item>
                    )}
                  ...
                  <div className='grid gap-2 md:block'>
                    <div>
                      {!resultItem.nonRefundable && (
                        <div className='mt-2 inline-flex items-center gap-2 rounded-md border border-gray-300 bg-gray-200 p-1 px-2 text-sm font-medium text-teal-800'>
                    ...
                    <div>
                      {(searchParams.isFlight || searchParams.isTransfer) && (
                        <List className='mx-0 mt-3 inline-flex items-center gap-2 rounded-md border p-1 px-2 pt-2 text-sm text-blue-800'>
                          {searchParams.isFlight && (
                            <List.Item icon={<FaPlaneDeparture />}>
                              Uçak Bileti
                            </List.Item>
                          )}
                          {searchParams.isFlight && searchParams.isTransfer && (
            ...
            <div className='mt-auto flex items-center px-5 text-start md:grid md:px-0 md:text-end'>
              <div className='grid'>
                <div className='flex items-center gap-2 text-blue-800'>
                  <List
                    size='xs'
                    className='mb-1 flex items-center gap-0.5 px-0 text-start text-blue-800 md:ms-auto'
                  >
                    <List.Item>Otel</List.Item>
                    {searchParams.isFlight && <List.Item>+ Uçak</List.Item>}
                    {searchParams.isTransfer && (
                      <List.Item>+ Transfer</List.Item>
                    )}
                  </List>
                </div>
                <div>
