...
import { buildAirlineCheckInLink } from './airline-links'
  ...
  const pnrInfoMutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response?.success && response.data) {
        const { airlineCode, surname, bookingCode, name } = response.data

        const airlineLink = buildAirlineCheckInLink(
          airlineCode,
          bookingCode,
          name || '',
          surname
        ...
        if (airlineLink) {
          window.location.href = airlineLink
        } else {
          notifications.show({
            title: 'Uyarı',
            message: 'Sonuç bulunamadı! Lütfen bilgileri kontrol ediniz.',
            color: 'yellow',
          })
        }
      }
