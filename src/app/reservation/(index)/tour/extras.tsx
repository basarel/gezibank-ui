          ...
          <div key={item.uniqueIdentifier}>
            {
              <Title order={5} pt={20} pb={5}>
                <span>
                  {extraDataType === 'BedType' &&
                    passengers.childNodes.some(
                      (passenger) => passenger.childNodes.length === 2
                    ) &&
                    'Yatak Seçimi'}
            ...
            {passengers.childNodes.map((passenger, passengerChildNodeIndex) => {
              const isBedType = extraDataType === 'BedType'

              return (
                <div key={passenger.orderId} className='grid grid-cols-2 gap-5'>
                  {passenger.childNodes.map((childNode, childNodeIndex) => {
