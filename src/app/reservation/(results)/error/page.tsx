...
const CheckoutErrorPage = async ({
  searchParams,
}: {
  searchParams: Promise<{ ProcReturnCode: number | string | null }>
}) => {
  const { ProcReturnCode } = await searchParams
    ...
    <div className='flex flex-col items-center gap-6 rounded-md py-4 md:px-10 md:py-10'>
      <Alert color='red' variant='light' className='grid w-full gap-3'>
        ...
        <div className='flex flex-col gap-4 text-center'>
          ...
          {ProcReturnCode && <div>VPOS{ProcReturnCode}</div>}
          <p className='text-lg text-gray-700'>
            Kart bilgilerini kontrol edip, tekrar deneyiniz.
          </p>
