...
import { CarSummaryResponse } from '@/app/(frontend)/reservation/types'
        ...
        <div className='grid gap-3'>
          ...
          <div className='grid grid-cols-10 py-3'>
            <div className='col-span-1 my-1 flex flex-col items-center'>
              ...
              <div className='h-15 w-0.5 bg-blue-600'></div>
              <div className='z-10 h-3 w-3 rounded-full bg-blue-800'></div>
            </div>
        ...
        <div className='text-sm'>
          ...
          >
            <div className='flex items-center gap-2'>
              ...
              <IoChevronDown
                size={20}
                className={`transition-transform duration-300 ${isPriceDetailOpen ? 'rotate-180' : ''}`}
              />
            </div>
          ...
          <Collapse in={isPriceDetailOpen}>
            <div className='grid gap-2 p-2'>
              ...
              <div className='flex justify-between gap-2 px-2 text-gray-600'>
              ...
              {selectedExtraOptions.length > 0 && (
                <div className='mt-2 grid gap-1 border-t pt-2'>
                  ...
                  {selectedExtraOptions.map((option, index) => (
                    <div
                      key={index}
                      className='flex justify-between px-2 text-gray-600'
                    >
                      <div>{option.name}</div>
              ...
              {selectedInsuranceOptions.length > 0 && (
                <div className='mt-2 grid gap-1 border-t pt-2'>
                  <div className='px-2 font-semibold text-gray-700'>
                    Sigorta Seçenekleri
                  </div>
                  {selectedInsuranceOptions.map((insurance, index) => (
                    <div
                      key={index}
                      className='flex justify-between px-2 text-gray-600'
                    >
                      ...
                      <div className='font-semibold'>
                        {formatCurrency(insurance.totalPrice.value)}
                      </div>
                    </div>
                  ))}
                </div>
