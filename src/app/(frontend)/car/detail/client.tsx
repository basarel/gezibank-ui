...
dayjs.extend(utc)
import { carDetailParams } from '@/app/(frontend)/car/searchParams'
import { serviceRequest } from '@/network'
import {
  CarExtraOption,
  CarInsuranceOption,
  DetailResponseData,
} from '@/app/(frontend)/car/detail/type'
...
import { RentTermsDrawer } from '@/app/(frontend)/car/component/rent-terms-drawer'
import { CarSearchResultItem } from '../component/result-item'
import { WorkingHoursDrawer } from '@/app/(frontend)/car/component/working-hour-drawers'
...
export const DetailClient = () => {
  ...
  const stepLabelsData = [
    { label: 'Araç Seçimi' },
    { label: 'Ekstra Ürünler ve Güvenceler' },
      ...
      <div className='my-3 hidden md:block'>
        <Breadcrumb
          items={[
            ...
            { title: `${detailItem.carDetail.name}` },
          ]}
        />
      </div>
      <div className='flex flex-col items-start gap-5 md:grid md:grid-cols-6'>
        <div className='col-span-4 grid gap-4'>
          ...
          {detailItem &&
            ...
            detailItem.carExtraOption.length > 0 &&
            detailItem.carExtraOption.filter((item) => item.isSelectable)
              .length > 0 && (
              <div className='flex flex-col gap-3 rounded-md border p-4'>
                ...
                <div className='text-xs'>
                  Aşağıdaki seçeneklerden herhangi birini talep edebilirsiniz.
                  Bu talepler garanti edilemez ve Ek hizmetlerin ücreti araç
                  teslimi sırasında firmaya ödenir.
                </div>
                <Spoiler
                  maxHeight={245}
                        ...
                        return (
                          <div key={extraOptionIndex}>
                            <Checkbox.Card
                              ...
                              className={clsx({
                                'border-blue-600': extraOption.selected,
          ...
          {detailItem &&
            ...
            detailItem.carInsurances.length > 0 &&
            detailItem.carInsurances.filter((item) => item.isSelectable)
              .length > 0 && (
              <div className='flex flex-col gap-3 rounded-md border p-4'>
                <Title order={3} fz='lg'>
                    ...
                    .map((insuranceOption, insuranceOptionIndex) => (
                      <div key={insuranceOptionIndex}>
                        <Checkbox.Card
                          ...
                          className={clsx({
                            'border-blue-600': insuranceOption.selected,
                        ...
                        >
                          <Group>
                            ...
                            <div className='md:text-md flex flex-1 items-center justify-between text-sm font-medium'>
                              ...
                              <div className='ms-auto font-semibold text-blue-700'>
                                {formatCurrency(
                                  insuranceOption.totalPrice.value
                                )}
                              </div>
                            </div>
                          </Group>
