...
import { RangeSlider, RangeSliderValue, UnstyledButton } from '@mantine/core'
...
const HourRangeSlider: React.FC<IProps> = ({
  ...
  const values: RangeSliderValue = [
    receivedValues.length > 0 ? receivedValues[0]?.value : 0,
    receivedValues.length > 1 ? receivedValues[1].value : lastItemInHourIndex,
  ]

  const handleOnChange = (value: RangeSliderValue) => {
    const selectedRange = marks.filter((mark) => value.includes(mark.value))
