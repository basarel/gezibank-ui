...
import { type HotelSearchRequestParams } from '@/types/hotel'
import { type GetSecurityTokenResponse } from '@/types/global'
import {
  type HotelCampaignsResponse,
  type HotelSearchResultApiResponse,
...
export const useSearchResultParams = ({ slug }: SlugParams) => {
  ...
  const [, setLocalParams] = useLocalStorage<HotelSearchEngineSchemaType>({
  ...
  const hotelCampaignsQuery = useQuery({
    enabled: !!searchRequestParams?.hotelSearchModuleRequest,
    queryKey: [
      'hotel-search-campaigns',
      searchRequestParams?.hotelSearchModuleRequest?.countryCode ?? 'tr',
    ],
    queryFn: async () => {
      const cc =
        searchRequestParams?.hotelSearchModuleRequest?.countryCode ?? 'tr'
