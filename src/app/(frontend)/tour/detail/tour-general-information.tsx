...
const convertBrToListItem = (html: string): string => {
  if (!html || html.includes('<ul>') || html.includes('<li>')) {
    return html
  }

  // <br/> tag'leri ile ayrılmış içerikleri liste formatına çevir
  const items = html.split(/<br\s*\/?>/i).filter((item) => item.trim())

  if (items.length > 1) {
    return `<ul class="list-disc list-inside space-y-2 ml-4">${items.map((item) => `<li>${item.trim()}</li>`).join('')}</ul>`
  }
