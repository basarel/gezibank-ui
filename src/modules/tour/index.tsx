...
const defaultDates = [dayjs().toDate(), dayjs().add(1, 'year').toDate()]
...
const TourSearchEngine: React.FC<Partial<TourSearchEngineSchemaType>> = ({
  destination,
  checkinDate,
  checkoutDate,
}) => {
  ...
  const formActions = useForm<TourSearchEngineSchemaType>({
    ...
    defaultValues: {
      checkinDate: checkinDate ?? localParams.checkinDate,
      checkoutDate: checkoutDate ?? localParams.checkoutDate,
      destination: {
        name: destination?.name ?? localParams.destination.name,
        slug: destination?.slug ?? localParams.destination.slug,
      },
    },
  })
    ...
    useQuery<LocationResults>({
      ...
      queryFn: async () => {
        const getLocations = await request({
          url: `${process.env.NEXT_PUBLIC_API_DESTINATION_ROUTE}/v1.1/api/tour/search`,
          params: {
  ...
  const handleSubmit = async (data: TourSearchEngineSchemaType) => {
    setLocalParams(data)
