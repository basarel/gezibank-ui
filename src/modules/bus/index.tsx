...
import { Button, Radio, Skeleton, UnstyledButton } from '@mantine/core'
...
const BusSearchEngine = () => {
  ...
  const [localStorageData, setLocalStorageData] =
    useLocalStorage<BusSearchEngineInfer>({
      ...
      defaultValue: {
        Date: dayjs().utc().startOf('day').add(1, 'd').toDate(),
        Destination: {
          Id: '0',
          Name: '',
          Slug: '',
  ...
  const formActions = useForm<BusSearchEngineInfer>({
    ...
    defaultValues: {
      ...localStorageData,
      Date: dayjs().utc().isAfter(localStorageData.Date, 'day')
        ? dayjs().utc().startOf('day').add(1, 'd').toDate()
        : dayjs(localStorageData.Date).utc().startOf('day').toDate(),
    },
  })
  ...
  const handleSubmit = async (data: BusSearchEngineInfer) => {
    ...
    queryClient.invalidateQueries({ queryKey: ['bus-search-session-tokens'] })

    router.push(url)
  }
  const watchedOriginLocation = useWatch({
    control: formActions.control,
    name: 'Origin',
  })
  const watchedDestinationLocation = useWatch({
    control: formActions.control,
    name: 'Destination',
  })
  const switchWay = () => {
    ...
    setPickupLocation(destination.Slug)
    setTargetLocation(origin.Slug)
      ...
      <div className='grid grid-cols-12 gap-3 md:grid-cols-25 md:gap-3'>
        <div className='relative col-span-12 grid gap-3 md:col-span-15 md:grid-cols-14'>
          <div className='relative col-span-7'>
            ...
            <BusLocations
              key={
                watchedOriginLocation?.Slug ??
                watchedOriginLocation?.Name ??
                'origin'
              }
          ...
          <div className='relative col-span-7'>
            ...
            <BusLocations
              key={
                watchedDestinationLocation?.Slug ??
                watchedDestinationLocation?.Name ??
                'destination'
              }
              label='Nereye'
          ...
          <div className='flex flex-row gap-8 md:flex-col md:gap-3'>
            ...
            <UnstyledButton
              ...
              onClick={() => {
                formActions.setValue(
                  'Date',
                  dayjs().utc().add(1, 'day').startOf('days').toDate()
                )
                formActions.trigger('Date')
              }}
            >
