...
import { Checkbox, NativeSelect, Skeleton } from '@mantine/core'
    ...
    <form className='block' onSubmit={formActions.handleSubmit(onSubmit)}>
      <div className='grid grid-cols-16 gap-3 md:grid-cols-25 md:gap-3'>
        ...
        <div className='col-span-16 md:col-span-7'>
          <CarCalendar
            defaultDates={[
              formActions.getValues('pickup_date'),
              formActions.getValues('drop_date'),
            ]}
            onDateSelect={(dates) => {
              if (dates[0]) {
                formActions.setValue(
                  'pickup_date',
                  dayjs.utc(dates[0]).toDate()
                )
                formActions.trigger('pickup_date')
              }
              if (dates[1]) {
                formActions.setValue('drop_date', dayjs.utc(dates[1]).toDate())
                formActions.trigger('drop_date')
              }
            }}
          />
        </div>
