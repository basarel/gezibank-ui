...
// https://destinationg.lidyateknoloji.com/v1.1/api/hotel/getthemebyslug?slug=kayak-otelleri
export const useGetThemeBySlug = (slug: string) =>
  useQuery({
    queryKey: ['hotel-theme-slug', slug],
    queryFn: async () => {
      const response = (await request({
        url: `${process.env.NEXT_PUBLIC_API_DESTINATION_ROUTE}/v1.1/api/hotel/getthemebyslug`,
        params: {
          slug,
        },
      })) as { Result: LocationResult }
