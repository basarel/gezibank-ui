...
const EbultenForm = () => {
  ...
  const submitMutiation = useMutation({
    ...
    onSuccess: (response) => {
      if (response?.success) {
        notifications.show({
          title: 'E-bülten kaydınız başarıyla alınmıştır.',
          message: <div> Teşekkür ederiz.</div>,
          position: 'top-right',
          color: 'green',
          classNames: {
            root: 'bg-green-50',
  ...
  return (
    <Box className='mt-10 rounded-2xl bg-gray-600 p-6 text-white shadow-lg md:p-10'>
      <div className='flex flex-col gap-6 md:flex-row md:items-center md:justify-between'>
        <div className='flex flex-col items-start gap-4 md:flex-row md:items-center'>
          <div className='rounded-full bg-white p-3'>
            <CiMail size={50} className='text-orange-800' />
          </div>
          <div>
        ...
        <div className='flex flex-col gap-2'>
          ...
          <Text size='xs' className='text-white/80'>
            Kullanım şartları ve genel koşullar için
            <Link
